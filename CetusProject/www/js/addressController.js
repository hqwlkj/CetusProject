...
.controller('AddresssCtrl', function($scope,$ionicPopover, $http,$stateParams, $state, $ionicLoading,$ionicHistory,$ionicActionSheet, $timeout,$ionicModal,$ionicPopup,ApiEndpoint, Userinfo) {
	...
	$scope.save_address = function(){
		var msg = "uid="+Userinfo.l.id+"&id="+$scope.addressObj.id;
		if(!$scope.address.recipients){
			$scope.showMsg("收件人为空");
		    return;
		}
		msg += "&recipients="+$scope.address.recipients;
		var reg = /^0?1[3|4|5|8][0-9]\d{8}$/;
		if (!reg.test($scope.address.telephone)) {
			$scope.showMsg("手机号码有误~");
		    return;
		};
		msg += "&telephone="+$scope.address.telephone;
		if(!$scope.address.province){
			$scope.showMsg("省为空");
		    return;
		}
		msg += "&province="+$scope.address.province;
		if(!$scope.address.city){
			$scope.showMsg("市为空");
			return;
		}
		msg += "&city="+$scope.address.city;
		if(!$scope.address.county){
			$scope.showMsg("县为空");
			return;
		}
		msg += "&county="+$scope.address.county;
		
		msg += "&zipCode="+$scope.address.zipCode;
		
		if(!$scope.address.locations){
			$scope.showMsg("详细地址为空");
		    return;
		}
